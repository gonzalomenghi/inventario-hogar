...
- `screens/AgregarProductoModal.tsx` — bottom sheet: busca en `productos_base` (debounce 300ms, `ilike` por nombre) o crea uno nuevo (categoría/unidad/marca/código de barras) y en el mismo paso lo agrega a `inventario_hogar` del usuario. Si el producto ya está en su inventario, el insert falla por la constraint única `(user_id, producto_id)` y se lo indica al usuario para que lo ajuste desde la lista en vez de duplicarlo.
